...
def evaluate(dataset, model, args, name="Validation", max_num_examples=None):
    ...
    result = {
        "prec": metrics.precision_score(labels, preds, average="macro", labels=np.unique(preds)),
        "recall": metrics.recall_score(labels, preds, average="macro", labels=np.unique(preds)),
        "acc": metrics.accuracy_score(labels, preds, labels=np.unique(preds)),
    }
...
def evaluate_node(ypred, labels, train_idx, test_idx):
    ...
    result_train = {
        "prec": metrics.precision_score(labels_train, pred_train, average="macro", labels=np.unique(pred_train)),
    ...
    result_test = {
        "prec": metrics.precision_score(labels_test, pred_test, average="macro", labels=np.unique(pred_test)),
