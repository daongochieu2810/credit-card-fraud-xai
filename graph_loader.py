...
class NaiveHetGraph(object):
    ...
    def __init__(self, node_type: Dict[int, str], edge_list: Tuple[int, int, str],
        ...
        with timeit(self.logger, 'node-enc-init'):
            self.node_encode = dict((n, i)
                                    for i, n in enumerate(node_type.keys()))
            self.node_decode = dict((i, n)
                                    for n, i in self.node_encode.items())
        ...
        with timeit(self.logger, 'seed-label-init'):
            self.seed_label_encoded = dict(
                (nenc[k], v) for k, v in seed_label.items())
    ...
    def get_sage_sampler(self, seeds, sizes=[-1], shuffle=False, batch_size=0):
        from .torch_sampler import NeighborSampler
        g = self
        g.node_type_encode
...
class NaiveHetDataLoader(object):
    ...
    def iter_sage(self):
        if self.cache_result and self.cache and len(self.cache) == len(self):
            ...
            for encoded_seeds in dl:
                batch_size, encoded_node_ids, adjs = sampler.sample(
                    encoded_seeds)
                ...
                if self.cache_result:
                    self.cache.append(
                        [encoded_seeds, encoded_node_ids, edge_ids])
    ...
    def get_node_type_weight_tensor(self):
        ...
        node_types = np.asarray([g.node_type[g.node_decode[i]]
                                for i in range(len(g.node_type))])
    ...
    def get_sage_neighbor_sampler(self, seeds):
        ...
        if self.method in {'sage', 'sage-merged'}:
            return NeighborSampler(
                ...
                node_idx=node_idx, num_nodes=len(g.node_type),
                batch_size=sum(
                    self.batch_size) if not self.seed_epoch else self.batch_size,
...
class DataLoader(object):
    ...
    def sample(self):
        ...
        if self.seed_epoch:
            ...
            logger.info('batch-size %d, #batch %d.',
                        batch_size, len(target_ids_chunk))
        else:
            candidates0 = list(
            ...
            for i in range(times):
                target0_ids = np.random.choice(
                    candidates0, size0, replace=False)
                target1_ids = np.random.choice(
                    candidates1, size1, replace=False)
                target_ids_chunk.append(
                    np.concatenate([target0_ids, target1_ids]))
